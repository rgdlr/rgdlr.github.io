...
function calculateBudget() {
    ...
    if (!location) {
        ...
        displacement = displacement[1] ? parseInt(displacement[0]) * 60 + parseInt(displacement[1]) : parseInt(displacement[0]);
    }

    duration = duration.split(":");
    duration = parseInt(duration[0]) + parseInt(duration[1]) / 60;

    let budget = recording * 90 + displacement * 0.5 + duration * 20 + edition * 20 + illumination * 15 + drone * 50 + script * 25 + coverYoutube * 15 + coverSpotify * 15 + photography * 25 + teaser * 20 + makingOf * 25;
...
function initMap() {
    ...
    google.maps.event.addListener(autocomplete, 'place_changed', function () {
        ...
        const originLatLng = { lat: 40.394291, lng: -3.712440 };
        ...
        distanceMatrixService.getDistanceMatrix(request).then((response) => {
            ...
            document.getElementById("displacement").innerText = response.rows[0].elements[0].duration.text.replace(/[^0-9\s]/g, '').replace(/\s{2,}/g, ' ');
            calculateBudget();
        });
...
function uploadFile() {
    ...
    var dbx = new Dropbox.Dropbox({ accessToken: 'wDcMa-Jc35IAAAAAAAAAAadab0gWZ5w1rfNTzZsmGS-b9BsVlvwayJE_UuBGJQvX' });
